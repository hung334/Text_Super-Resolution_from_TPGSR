...
import string
from myself_text_dataloader import myself_text_datasets_demo
...
def str_filt_paddle(str_, dict_character):

    for char in str_:
        if char not in dict_character: #voc_type
...
def parse_paddleocr_data(batch_x,detect=False):
    
        def narrow_224_32(image, expected_size=(280,48)):
            ih, iw = image.shape[2:4]
            ew, eh = expected_size
            scale = eh / ih
            nh = int(ih * scale)
            nw = int(iw * scale)
            new_image = torch.nn.functional.interpolate(image,(nh,nw), mode='bicubic')
            top = 0
            bottom = eh - nh - top
            left = 0
            right = ew - nw - left
            pad_img = F.pad(new_image,(left,top,right,bottom),mode='constant',value=(114/255))
            return pad_img
        
        b_size,img_c,img_h,img_w = batch_x.shape
        
        resize_h_ratio = 48 / img_h #計算高的縮小倍率
        new_batch_x = torch.nn.functional.interpolate(batch_x,scale_factor=resize_h_ratio, mode='bicubic')
        #print(new_batch_x.shape)
        parse_img = new_batch_x
        if(detect):
        
            n_b_size,n_img_c,n_img_h,n_img_w = new_batch_x.shape
            W = math.ceil(new_batch_x.shape[-1]/32)+1
            parse_img = narrow_224_32(new_batch_x,expected_size=(W*32,48))
...
class TextSR(base.TextBase):
    ...
    def train(self):
        ...
        dict_path = r"./Paddle_OCR_weights/ppocr_keys_v1.txt"
        converter_CTC = CTCLabelConverter(dict_path)
        dict_character = []
        ...
        if self.args.myself_data:
            if self.args.use_myself_aug :
                #train_dataset, train_loader = self.get_train_myself_dat_aug_case1()
                train_dataset, train_loader = self.get_train_myself_dat_aug_case1_multisize()#標準版
                #train_dataset, train_loader = self.get_train_textzoom_pdfimg()
                #train_dataset, train_loader = self.get_train_Diversity_myself_data()
            else:
                train_dataset, train_loader = self.get_train_myself_data()
                
            val_dataset_list, val_loader_list = self.get_val_myself_data() #標準版
            #val_dataset_list, val_loader_list = self.get_val_myself_data_textzoom_easy()
        else:
        ...
        Paddle_OCR = self.Paddle_OCR_init(use_cuda=True) #新增~~
        Paddle_OCR.eval() #新增~~
        ...
        print("self.args.myself_data",self.args.myself_data)
        if not self.args.myself_data:
            ...
            best_history_acc = {'myself_data':0}
        print("best_history_acc-----------------",best_history_acc )
            ...
            for j, data in (enumerate(train_loader)):
                ...
                if not self.args.go_test:
                    ...
                    if self.args.syn:
                        ...
                        if self.args.arch == "tsrn":
                        ...
                        elif self.args.myself_data :
                            images_hr, images_lr, label_strs = data
                            images_lr = torch.nn.functional.interpolate(images_lr.clone(), scale_factor=0.5, mode='bicubic')
                            ...
                            #print("images_hr.shape",images_hr.shape)
                            #print("images_lr.shape",images_lr.shape)
                    ...
                    if self.args.arch == "tsrn":
                        ...
                        #print("self.args.use_label",self.args.use_label)
                        
                        #print('input_img.shape:',images_hr[:, :3, :, :].shape)
                        #aster_dict_hr = self.parse_crnn_data(images_hr[:, :3, :, :])
                        #aster_dict_hr = images_hr[:, :3, :, :].clone()#self.parse_crnn_data(images_hr[:, :3, :, :])
                        #aster_dict_hr = torch.nn.functional.interpolate(images_hr[:, :3, :, :].clone(), scale_factor=2, mode='bicubic')
                        #輸入至PP-OCR的(32,128)->(64,256)
                        
                        if self.args.use_paddleocr:
                            aster_dict_hr = parse_paddleocr_data(images_hr[:, :3, :, :].clone())
                            #print(aster_dict_hr)
                            #label_vecs_logits_hr = Paddle_OCR(aster_dict_hr)
                            label_vecs_logits_hr = Paddle_OCR(aster_dict_hr).detach()#
                        else:
                            aster_dict_hr = self.parse_crnn_data(images_hr[:, :3, :, :])
                            label_vecs_logits_hr = aster(aster_dict_hr).detach()
                        
                        #print('aster_dict_hr.shape:',aster_dict_hr.shape)
                        #label_vecs_logits_hr = aster(aster_dict_hr).detach()
                        #label_vecs_logits_hr = Paddle_OCR(aster_dict_hr).detach()#新增~~
                        label_vecs_hr = torch.nn.functional.softmax(label_vecs_logits_hr, -1)
                        #print("label_vecs_logits_hr.shape:",label_vecs_logits_hr.shape)
                        ...
                        for i in range(self.args.stu_iter):
                            ...
                            #print("cascade_images.shape",cascade_images[:, :3, :, :].shape)
                            #aster_dict_lr = self.parse_crnn_data(cascade_images[:, :3, :, :])
                            #aster_dict_lr = cascade_images[:, :3, :, :].clone()
                            #aster_dict_lr = torch.nn.functional.interpolate(cascade_images[:, :3, :, :].clone(), scale_factor=2, mode='bicubic')
                            
                            #aster_dict_lr = torch.nn.functional.interpolate(cascade_images[:, :3, :, :].clone(), (64, 256), mode='bicubic')
                            #aster_dict_lr = torch.nn.functional.interpolate(cascade_images[:, :3, :, :].clone(), scale_factor=4, mode='bicubic')
                            #輸入至PP-OCR的(16,64)->(64,256)
                            
                            if self.args.use_paddleocr:
                                aster_dict_lr = parse_paddleocr_data(cascade_images[:, :3, :, :].clone())
                                label_vecs_logits = Paddle_OCR(aster_dict_lr).detach()#新增~~
                            else:
                                aster_dict_lr = self.parse_crnn_data(cascade_images[:, :3, :, :])
                                label_vecs_logits = stu_model(aster_dict_lr)
                            
                            #print("aster_dict_lr.shape",aster_dict_lr.shape)
                            #label_vecs_logits = stu_model(aster_dict_lr) #stu_model 也是個 CRNN
                            #label_vecs_logits = Paddle_OCR(aster_dict_lr).detach()#新增~~
                            #print("label_vecs_logits.shape",label_vecs_logits.shape)
                            label_vecs = torch.nn.functional.softmax(label_vecs_logits, -1)
                            #print("label_vecs.shape",label_vecs.shape)
                            #label_vecs = label_vecs.permute(1, 0, 2)# 新增的
                            label_vecs_final = label_vecs.permute(1, 0, 2).unsqueeze(1).permute(0, 3, 1, 2)
                            #print("label_vecs_final.shape",label_vecs_final.shape)
                            '''
                            #####################################################
                            # Sample shift
                            # [N, C, H, W]
                            ...
                            '''
                            # image for cascading
                            if self.args.sr_share:#True
                                pick = 0
                            else:
                                pick = i
                            
                            if self.args.use_label: #False
                                # [B, L]
                            ...
                            #images_lr = torch.nn.functional.interpolate(images_lr[:, :, :, :].clone(), scale_factor=4, mode='bicubic')
                            #(16,64)->(32,)
                            ...
                            #print("images_lr.shape",images_lr.shape)
                            label_vecs_final = label_vecs_final.permute(3, 1, 2,0)#新增~
                            #print("label_vecs_final.shape",label_vecs_final.shape)
                            #print(model_list[pick])
                            cascade_images = model_list[pick](images_lr, label_vecs_final)
                            #print("cascade_images:",cascade_images.shape)
                            ...
                            if iters % 5 == 0 and i == self.args.stu_iter - 1:
                                if iters % 100 == 0:
                                    ...
                                    if self.args.use_paddleocr:
                                        ...
                                        #三通道圖
                                        paddle_parse_sr_grid = torchvision.utils.make_grid(cascade_images[:, :3, :, :])
                                        self.results_recorder.add_image('train/paddle_parse_sr_images', paddle_parse_sr_grid, global_step=iters)
                                        paddle_parse_lr_grid = torchvision.utils.make_grid(aster_dict_lr)
                                        self.results_recorder.add_image('train/paddle_parse_lr_images', paddle_parse_lr_grid, global_step=iters)
                                        paddle_parse_hr_grid = torchvision.utils.make_grid(aster_dict_hr)
                                        self.results_recorder.add_image('train/paddle_parse_hr_images', paddle_parse_hr_grid, global_step=iters)
                                    '''
                                    #四通道圖
                                    sr_grid = torchvision.utils.make_grid(cascade_images)
                                    self.results_recorder.add_image('train/sr_images', sr_grid, global_step=iters)
                                    lr_grid = torchvision.utils.make_grid(images_lr)
                                    self.results_recorder.add_image('train/lr_images', lr_grid, global_step=iters)
                                    hr_grid = torchvision.utils.make_grid(images_hr)
                                    self.results_recorder.add_image('train/hr_images', hr_grid, global_step=iters)
                ...
                if iters % cfg.VAL.valInterval == 0 or self.args.go_test:
                    ...
                    for k, val_loader in enumerate(val_loader_list):
                        data_name = list(best_history_acc.keys())[k]
                        ...
                        metrics_dict = self.eval(
                            ...
                            aster_info,
                            Paddle_OCR,
                            converter=converter_CTC,
                            dict_character=dict_character,
                            epoch=epoch
                        )

                        
                        #self.test_myself(model_list,Paddle_OCR,iters)
                    ...
                    if sum(current_acc_dict.values()) > best_acc:
                        ...
                        #self.training_demo(model_list,Paddle_OCR,epoch, iters)
    ...
    def eval(self, model_list, val_loader, image_crit, index, aster, aster_info,Paddle_OCR,converter,dict_character,epoch):
        cfg = self.config.TRAIN
        ...
        for i, data in (enumerate(val_loader)):
            SR_stat = []
            SR_tick = False
            iters = len(val_loader) * epoch + i + 1
            if self.args.syn:
                images_hr, images_lr, label_strs, identity = data
            ...
            if self.args.arch == "tsrn":
                ...
                if self.args.random_reso:
                    ...
                    # Get char mask
                    with torch.no_grad():#主要是這裡~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                        # aster_dict_hr = self.parse_crnn_data(images_hr[:, :3, :, :])
                        # label_vecs_hr = aster[1][0](aster_dict_hr)
                        
                        #aster_dict_hr = images_hr[:, :3, :, :].clone()
                        #aster_dict_hr = torch.nn.functional.interpolate(images_hr[:, :3, :, :].clone(), scale_factor=2, mode='bicubic')
                        #輸入至PP-OCR的(32,128)->(64,256)
                        aster_dict_hr = parse_paddleocr_data(images_hr[:, :3, :, :].clone())
                        label_vecs_hr = Paddle_OCR(aster_dict_hr).detach()#新增~~
                    ...
                    #******************************************
                    val_loss_img = 0.
                    val_loss_recog_distill = 0.
                    #******************************************
                    
                    for ii in range(self.args.stu_iter):
                        ...
                        if self.args.use_distill:
                                ...
                                label_vecs_hr = torch.nn.functional.softmax(label_vecs_hr, -1)
                                loss_recog_distill_each = sem_loss(label_vecs, label_vecs_hr) * 100
                                val_loss_recog_distill += loss_recog_distill_each  # * (1 + 0.5 * i)
                        loss_img_each = image_crit(cascade_images, images_hr).mean() * 100
                        val_loss_img += loss_img_each
                        
                        if self.args.ssim_loss:
                                loss_ssim = (1 - ssim(cascade_images, images_hr).mean()) * 10.
                                val_loss_img += loss_ssim
                        #******************************************************************************
                        if iters % 100 == 0:
                            '''
                            if self.args.use_paddleocr:
                                        #三通道圖
                                        paddle_parse_sr_grid = torchvision.utils.make_grid(cascade_images[:, :3, :, :])
                                        self.results_recorder.add_image('eval/paddle_parse_sr_images', paddle_parse_sr_grid, global_step=iters)
                                        paddle_parse_lr_grid = torchvision.utils.make_grid(aster_dict_lr)
                                        self.results_recorder.add_image('eval/paddle_parse_lr_images', paddle_parse_lr_grid, global_step=iters)
                                        paddle_parse_hr_grid = torchvision.utils.make_grid(aster_dict_hr)
                                        self.results_recorder.add_image('eval/paddle_parse_hr_images', paddle_parse_hr_grid, global_step=iters)
                            '''
                            #四通道圖
                            sr_grid = torchvision.utils.make_grid(cascade_images)
                            self.results_recorder.add_image('eval/sr_images', sr_grid, global_step=iters)
                            lr_grid = torchvision.utils.make_grid(images_lr)
                            self.results_recorder.add_image('eval/lr_images', lr_grid, global_step=iters)
                            hr_grid = torchvision.utils.make_grid(images_hr)
                            self.results_recorder.add_image('eval/hr_images', hr_grid, global_step=iters)
                        #*********************************************************************************************
                    loss_im = val_loss_img + val_loss_recog_distill
                    ...
                    if iters % cfg.displayInterval == 0:
                        print('eval   [{}]\t'
                              ...
                              .format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                      ...
                                      self.vis_dir,
                                      float(loss_im.data),
                                      float(val_loss_img.data),
                                      float(loss_ssim),
                                      float(val_loss_recog_distill.data)
                                      ))
            ...
            if self.args.use_paddleocr_val:
                aster_dict_hr = parse_paddleocr_data(images_lr[:, :3, ...])
                aster_dict_hr = parse_paddleocr_data(images_hr[:, :3, ...])
            else:
            ...
            if self.args.test_model == "MORAN":
                ...
                if self.args.use_paddleocr_val:
                    ...
                    aster_output_lr = Paddle_OCR(aster_dict_lr).softmax(dim=-1)
                    aster_output_hr = Paddle_OCR(aster_dict_hr).softmax(dim=-1)
                else:
            ...
            #print("type(images_sr)",type(images_sr))
            if type(images_sr) == list:#是list 
                predict_result_sr = []
                ...
                if self.args.arch in ABLATION_SET:
                    ...
                    for i in range(self.args.stu_iter):
                        ...
                        if self.args.use_paddleocr_val:
                            aster_dict_sr = parse_paddleocr_data(image[:, :3, ...])
                        ...
                        if self.args.test_model == "MORAN":
                            ...
                            if self.args.use_paddleocr_val:
                                aster_output_sr = Paddle_OCR(aster_dict_sr).softmax(dim=-1)
                        ...
                        if self.args.test_model == "CRNN":
                            ...
                            if self.args.use_paddleocr_val:
                                aster_output_sr = aster_output_sr.cpu().data
                                predict_result_sr_ = converter.decode(aster_output_sr.detach().cpu().numpy())
                                predict_result_sr_ = [data[0] for data in predict_result_sr_]
                                #print('predict_result_sr_',predict_result_sr_)

                            else:
                                predict_result_sr_ = aster[0]["string_process"](aster_output_sr)
                                print('crnn_predict_result_sr_',predict_result_sr_)
                ...
                #print("psnr,img_sr,img_hr",img_sr.shape, img_hr.shape)
            ...
            if self.args.test_model == "CRNN":
                ...
                if self.args.use_paddleocr_val:
                    ...
                    aster_output_lr = aster_output_lr.cpu().data
                    aster_output_hr = aster_output_hr.cpu().data
                    predict_result_lr = converter.decode(aster_output_lr.detach().cpu().numpy())
                    predict_result_hr = converter.decode(aster_output_hr.detach().cpu().numpy())
                    
                    predict_result_lr = [data[0] for data in predict_result_lr ]
                    predict_result_hr = [data[0] for data in predict_result_hr ]
                    
                    #predict_result_lr = predict_result_lr[0][0]
            ...
            #print("len(predict_result_sr)",len(predict_result_sr))
            
            # print("predict_result_sr",predict_result_sr)
            # print("predict_result_lr",predict_result_lr)
            # print("predict_result_hr",predict_result_hr)
            ...
            for batch_i in range(len(images_lr)):
                ...
                label = str_filt(label, 'lower') if not self.args.use_paddleocr_val else label
                #print('label',label)
                if self.args.arch in ABLATION_SET:
                    #print("self.args.stu_iter",self.args.stu_iter)
                    for k in range(self.args.stu_iter):
                            
                            #print("predict_result_sr[k][batch_i]",predict_result_sr[k][batch_i])
                            if predict_result_sr[k][batch_i] == label:
                                counters[k] += 1

                    # print("predict_result_sr:", predict_result_sr)
                    
                    if predict_result_sr[-1][batch_i] == label :#str_filt(label, 'lower'):
                        sr_wrong = False

                else:
                    if predict_result_sr[batch_i] == label :#str_filt(label, 'lower'):
                        sr_wrong = False
                        n_correct += 1
                    else:
                        iswrong = True
                #print("len(predict_result_lr)",len(predict_result_lr))
                if predict_result_lr[batch_i] == label :#str_filt(label, 'lower'):
                    lr_wrong = False
                    n_correct_lr += 1
                else:
                    iswrong = True
                if predict_result_hr[batch_i] == label :#str_filt(label, 'lower'):
                    hr_wrong = False
                    n_correct_hr += 1
                else:
                    iswrong = True
    ...
    def test(self):
        ...
        for i, data in (enumerate(test_loader)):
            ...
            print("data[2]",data[2])
            print("data[3]",data[3].shape)
            print(self.device)
            #print("data[3]",data[3])
            #print(len(data))
            images_hr, images_lr, label_strs ,label_tensor = data
            val_batch_size = images_lr.shape[0]
            images_lr = images_lr#.to(self.device)
            images_hr = images_hr#.to(self.device)
            sr_beigin = time.time()
    ...
    def demo_myself(self):
        cfg = self.config.TRAIN
        from torch.autograd import Variable
        mask_ = self.args.mask
        def transform_(path):
            img = Image.open(path)
            img = transforms.Resize(cfg.height, interpolation=Image.BICUBIC)(img)
            #img = img.resize((cfg.width,cfg.height), Image.BICUBIC)
        ...
        model_dict = self.generator_init_myself()
        model, image_crit = model_dict['model'], model_dict['crit']
        
        Paddle_OCR = self.Paddle_OCR_init(use_cuda=True)
        Paddle_OCR.eval() #新增~~
        ...
        for im_name in tqdm(os.listdir(self.args.demo_dir)):
            images_lr = transform_(os.path.join(self.args.demo_dir, im_name))
            images_lr = Variable(images_lr).cuda()#.to('cuda:0')#images_lr.to('cuda:0')
            images_lr = torch.nn.functional.interpolate(images_lr.clone(), scale_factor=0.5, mode='bicubic')
            sr_beigin = time.time()
            
            
            aster_dict_lr = parse_paddleocr_data(images_lr[:, :3, :, :].clone())
            label_vecs_logits = Paddle_OCR(aster_dict_lr).detach()#新增~~
            label_vecs = torch.nn.functional.softmax(label_vecs_logits, -1)
            #label_vecs_final = label_vecs.unsqueeze(0).permute(0, 3, 1, 2)
            label_vecs_final = label_vecs.permute(1, 0, 2).unsqueeze(1).permute(0, 3, 1, 2)
            label_vecs_final = label_vecs_final.permute(3, 1, 2,0)
            #print("label_vecs_final.shape",label_vecs_final.shape)
            images_sr = model(images_lr, label_vecs_final)
            #images_sr = model(images_lr)#.cuda()
            self.test_display_myself(images_lr.cpu(), images_sr.cpu(), '123', '123',im_name, 1) #testing
            sr_end = time.time()
            sr_time += sr_end - sr_beigin
            
            # crnn_input = self.parse_crnn_data(images_sr[:, :3, :, :])
            # crnn_output = crnn(crnn_input.to(self.device))
            # _, preds = crnn_output.max(2)
            # preds = preds.transpose(1, 0).contiguous().view(-1)
            # preds_size = torch.IntTensor([crnn_output.size(0)] * val_batch_size)
            # pred_str_sr = self.converter_crnn.decode(preds.data, preds_size.data, raw=False)
